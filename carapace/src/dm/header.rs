...
impl DmHeader {
    ...
    pub(super) fn new(name: &str) -> Result<Self, DmError> {
        ...
        if bytes.contains(&0) {
            return Err(DmError::Usage("dm device name contains NUL byte".into()));
        }
...
mod tests {
    ...
    fn header_new_rejects_nul_in_name() {
        assert!(matches!(DmHeader::new("foo\0bar"), Err(DmError::Usage(_))));
    }

    #[test]
    fn header_new_rejects_overlong_name() {
        let long = "a".repeat(128);
        assert!(matches!(DmHeader::new(&long), Err(DmError::Usage(_))));
    }
