...
fn write_hex_lower<W: std::fmt::Write>(w: &mut W, bytes: &[u8]) -> std::fmt::Result {
    for b in bytes {
        write!(w, "{b:02x}")?;
    }
    Ok(())
...
pub enum TargetSpec {
    ...
    Verity {
        ...
        hash_dev: (u32, u32),
        num_data_blocks: u64,
        /// The dm-verity hash algorithm *name* (e.g. `"sha256"`) — the
        /// token written verbatim into the kernel table line. Kept as a
        /// plain string so this crate stays chain-agnostic; the caller
        /// (`carapace::assemble`) supplies a validated algorithm name.
        algorithm: String,
        digest: Vec<u8>,
        salt: Vec<u8>,
    },
    ...
    fn write_params<W: std::fmt::Write>(&self, idx: usize, w: &mut W) -> std::fmt::Result {
        ...
        match &line.target {
            ...
            TargetSpec::Verity {
                ...
                write!(
                    w,
                    "1 {dmaj}:{dmin} {hmaj}:{hmin} {RDP_BLOCK_SIZE} {RDP_BLOCK_SIZE} {num_data_blocks} 1 {algorithm} ",
                )?;
...
impl DmTableBuf {
    ...
    /// allocated.
    pub(super) fn build(name: &str, table: &DmTable) -> Result<Self, DmError> {
...
mod tests {
    ...
    fn verity_ctor_uses_expected_root_and_renders_per_kernel_docs() {
        // 10 data blocks, salt 0xAA*32, expected root 0xBB*32 — the same
        // shape the old superblock fixture produced, now passed directly
        // (the dm layer is decoupled from the verity superblock parser).
    ...
    fn buf_for_verity_target_has_correct_layout_and_params() {
        // 7 data blocks, salt 0x55*32, expected root 0xCD*32 — passed
        // directly now that the dm layer is parser-decoupled.
        let t = DmTable {
            lines: vec![TableLine {
                start: 0,
                length: 56,
